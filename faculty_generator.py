...
top.title(" Faculty Id Card Generator")
...
branch = Label(top, text = "Branch").place(x = 30,y = 170)
name = Label(top, text = "name").place(x = 30,y = 140)
dob = Label(top, text = "DOB").place(x = 30,y = 200)
grno = Label(top, text = "GR No.").place(x = 30,y = 230)
doj = Label(top, text = "Date of Joining").place(x = 30,y = 260)
dor = Label(top, text = "Date of resiging").place(x = 30,y = 290) 
mob_num = Label(top, text = "Mobile Number").place(x = 30,y = 320)
#ddl = Label(top, text = "Drop Down List").place(x = 30,y = 320) 
...
#For Entry Widget
e1= ttk.Combobox(top,textvariable=str1,values=["Saraswati", "AC Patil", "Bharatiya", "wilson"]).place(x = 120, y = 80)# Add more colleges as needed
...
def register():
        ...
        if str1.get()==""or str2.get()==""or str3.get()=="":
            ...
            my_cursor=conn.cursor()
            query=("select * from faculty_information where id=%s")
            value=(str2.get(),)
            ...
            if row!=None:
                ...
                my_cursor.execute("insert into faculty_information (collge,id,name,branch,DOB,grno,date_of_join,date_of_resign,phone)values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                                                                                      str1.get(),
                                                                                      str2.get(),
                                                                                      str3.get(),
                                                                                      str4.get(),
           ...
           # messagebox.showinfo("Sucess","Register Sucessfully")
            ...
            #Name
            (x, y) = (50, 250)
            message = str('Name:- '+str(str3.get()))
            color = 'rgb(0, 0, 0)' # black color
            font = ImageFont.truetype('arial.ttf', size=45)
            draw.text((x, y), message, fill=color, font=font)
            #Gender
            (x, y) = (50, 350)
            message = str('branch:- '+str(str4.get()))
            color = 'rgb(0, 0, 0)' # black color 
            draw.text((x, y),message, fill=color, font=font)
            #Age
            (x, y) = (50, 450)
            message = str('DOB:- '+str(str5.get()))
            color = 'rgb(0, 0, 0)' # black color
            draw.text((x, y), message, fill=color, font=font)
            #DOB
            (x, y) = (50, 550)
            message = str('GrNo.:- '+str(str6.get()))
            draw.text((x, y),  message, fill=color, font=font)
            #Blood  Group
            (x, y) = (50, 650)
            message = str('DOJ:- '+str(str7.get()))
            color = 'rgb(0, 0, 0)' # black color 
            draw.text((x, y), message, fill=color, font=font)
            #Mobile Number
            (x, y) = (50, 750)
            message = str('DOR:- '+str(str8.get()))
            color = 'rgb(0, 0, 0)' # black color
            draw.text((x, y), message, fill=color, font=font)
            #Address
            (x, y) = (50, 850)
...
#b2 = tk.Button(top,text = "Submit",state=ACTIVE,command = submit,activeforeground = "red",activebackground = "pink",font="Baskerville_Old_Face").place(x=150,y=350)
